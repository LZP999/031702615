...
def Name(Location):
    Name=re.findall("!(\D*.*?),",Location)  #把，前面的名字提取出来)
    return Name

def address(Location):
    place=re.sub(r'\d{11}',"",Location) #先把电话号码删掉
    place=re.findall("\D*.*?,(.*?)\.",place) #从文件中提取出地址
    place=("".join(place))
    return(separate(place,difficulty(Location)))

#把分离出来的人名、电话号码、地址合并起来存在一个新的数组里
def result(Location):
    sum=[]
    num=phone(Location)
    name=Name(Location)
...
def separate(place,num):
    ...
    for i in range(len(sheng)):
        ...
        if(a):
            S=i
            one=a.group()
            place=re.sub(one,"",place,1) #匹配到后把省份名字去掉
            if i<4:      #当匹配到的是直辖市的时候
                two=one+'市'
                if re.match(r'市',place):
                    place=re.sub(r'市',"",place,1)  #把“市”字去掉
            if 8<i<31:  #当匹配到的是普通省份的时候
                one+="省"
                if re.match(r'省',place):
                    place=re.sub(r'省',"",place,1) #把“省”字去掉                
            break
    ...
    if S>3:
        if one!="": #即省份不缺失时
            for i in range(len(shi[S])):
                a=re.match(shi[S][i],place) 
                if a==None:#匹配不到时
                    two=""
                else:
                    two=a.group()
                    place=re.sub(two,"",place,1) #匹配到后把市级的名字去掉
                    two+="市"
                    if re.match(r'市',place):
                        place=re.sub(r'市',"",place)  #把“市”字去掉
                    break
    #匹配县级
    a=re.match(r'.*?(县|区|市)',place) #匹配县/区/县级市
    if a==None:
        three=""
    else:
        three=a.group()
        place=re.sub(three,"",place,1)
    #匹配街道/镇/乡
    a=re.match(r'.*?(街道|镇|乡)',place)
    if a==None:
        four=""
    else:
        four=a.group()
        place=re.sub(four,"",place,1)
    ...
    if num=='1':
        ...
        #匹配路名
        a=re.match(r'.*?(路|街)',place)
        if a==None:
            five=""
        else:
            five=a.group()
            place=re.sub(five,"",place,1)
        #匹配门牌号
        a=re.match(r'.*?号',place)
        if a==None:
            six=""
        else:
            six=a.group()
            place=re.sub(six,"",place,1)
        #匹配详细地址
        seven=place
        #将分级后的地址存入数组中
        sum=[]
